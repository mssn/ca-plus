            ...
            for i in range(0, len(logcontent)):
                ...
                if map_flag and 'MeasObj ' not in logline:
                    ...
                    if freq not in counter[car]:
                        counter[car][freq] = defaultdict(int)
                    if one_meas_state:
                        ...
                        if pcell_included or no_filter: # filter meas with serving cell
                            counter[car][freq][one_meas_state] += 1
                    freq_id_dict = {} # e.g., {'1': '9820_P', '2': '2175_S'}
                    report_dict = {} # e.g., {'1': 'type: a1, thr/ofst: 3.0 None, hetersis: 1.0, tt_trigger: 80'}
                    obj_flag, rpt_flag, map_flag = False, False, False
                    one_meas_state = set()
...
def printer(counter, min_cnt = 0):
    for car, freq_measstate_count in counter.items():
        print('Carrier: ' + car)
        # if car == '310410':
        for freq, measstate_cnt in freq_measstate_count.items():
            print('\tFrequency: ' + freq, 'Cnt:', len(measstate_cnt))
            for measstate, cnt in sorted(measstate_cnt.items(), key=lambda item: item[1], reverse=True):
            # for measstate, cnt in dict(sorted(measstate_cnt.items(), key=operator.itemgetter(1), reverse=True)).items():
                if cnt > min_cnt:
                    print('\t\tMeasurement state count: ' + str(cnt))
                    for cfg in measstate:
                        print('\t\t\t' + cfg)
